...
# install dependency
RUN apt-get update && apt-get install curl rlwrap -y
...
# run the commands which is written in https://clojure.org/guides/install_clojure
RUN curl -O https://download.clojure.org/install/linux-install-1.11.1.1273.sh
RUN chmod +x linux-install-1.11.1.1273.sh
RUN ./linux-install-1.11.1.1273.sh # there is no sudo command because this env is in root, but the original used

# run first setup on clojure repl
RUN clojure
